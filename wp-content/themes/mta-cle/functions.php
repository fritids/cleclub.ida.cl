...
function parseFeedHome($url, $cuantos = 3, $descripcion = false) {
    ...
    if ($maxitems == 0) :
        ...
        foreach ($rss_items as $item) :
            $clase = $i % 2 ? ' odd' : '';
            $desc = $descripcion ? '<p class="desc">' . $item->get_content() . '</p>' : "";
            $out .= '<div class="wNota clearfix"><div class="tituloNotaW"><small>' . __(strtolower($item->get_date('j \d\e ')), "cleclub") . __(strtolower($item->get_date('F')), "cleclub") .", ". __(strtolower($item->get_date('Y')), "es_ES") . '</small><a href="/noticias-df/?url=' . urldecode(esc_url($item->get_permalink())) . '&amp;titulo=' . urldecode(esc_html($item->get_title())) . '">' . esc_html($item->get_title()) . '</a></div>' . $desc . '</div>';
            $i++;
        endforeach;
        return $out;
    endif;
...
?>
>>>>>>> d81ee67677f71791aa2ece50c073b23c92971ad2
